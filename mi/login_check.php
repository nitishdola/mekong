...
else
{
	...
	$sql_stime = "select count(*) as tlock from `lastlogin_mi` where `user` ='$logUser' and status ='failed' and date='$regdate'";
	if($cn == false)
	connect3db();
	...
	if($tlock >= '5')
    {
		if($cn == false)
		connect3db();
		$rs_ltime = mysql_query("select ltime from `lastlogin_mi` where `user` ='$logUser' and status ='failed' and date='$regdate' order by id desc limit 1 ") or die(mysql_error());
	...
	foreach($user1 AS $user2)
	{
		foreach($symb1 AS $try1)
		{
			...
			if($pos1 !== false)
			{
				...
				if($cn == false)
				connect3db();
				mysql_query("INSERT INTO lastlogin_mi(user,date,ip,atmpt,status,ltime) VALUES('$user' ,'$regdate','$ip','1','failed','$ltime')") or die(mysql_error());			echo 'Incorrect User name and Password not allowed';
				 exit();
		 	}
			
		}
	} 
	$sql = "select * from mi_user where username = '".$logUser."'";
	if($cn == false)
	connect3db();
	$res = mysql_query($sql);
	if(mysql_num_rows($res))
	{
				...
				else
				{
					...
					if($cn == false)
					connect3db();
					mysql_query("INSERT INTO lastlogin_mi(user,date,ip,atmpt,status,ltime) VALUES('$logUser','$regdate','$ip','1','failed','$ltime')") or die(mysql_error());				exit();
	...
	else
	{
		...
		if($cn == false)
		connect3db();
		mysql_query("INSERT INTO lastlogin_mi(user,date,ip,atmpt,status,ltime) VALUES('$logUser','$regdate','$ip','1','failed','$ltime')") or die(mysql_error());		
		exit();	
	}
